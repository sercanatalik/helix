...
### Vega-Lite charts must render, not sit as JSON in a `<pre>` block

The single most common AI-document failure: pasting a Vega-Lite spec into `<pre><code>{...}</code></pre>` and shipping it. That ships text, not a chart — nothing paints. Always wire the spec to a live render. Every Vega-Lite chart in an HTML deliverable needs **all four** of these pieces:

1. **CDN scripts in `<head>`** (one set for the whole document, not per chart):
   ```html
...
2. **A target `<div>` with a stable, unique id** inside the `<figure>`:
   ```html
   <figure>
     <div id="vis-funding" class="vega-chart" style="width:100%"></div>
     <figcaption>...</figcaption>
   </figure>
   ```

3. **The spec in a JSON script block** (keeps the spec inspectable; the `type="application/json"` stops the browser from executing it as code):
   ```html
   <script type="application/json" id="vis-funding-spec">
   { "$schema": "https://vega.github.io/schema/vega-lite/v5.json",
     "mark": "line", "width": "container", ... }
   </script>
   ```

4. **An init call** (one per chart, near the end of `<body>` or in a single batch):
   ```html
   <script>
     vegaEmbed('#vis-funding', JSON.parse(
       document.getElementById('vis-funding-spec').textContent
     ), { actions: false });
   </script>
   ```

Rules:
- Use unique ids per chart on the page (`vis-<topic>` / `vis-<topic>-spec`). Two charts sharing an id silently break.
- Pass `{ actions: false }` to drop the export-menu icon — cleaner for editorial output. Omit only when the user asks for the export menu.
- Set `"width": "container"` in the spec and give the host `<div>` a width (`width: 100%` or a fixed pixel size) so the chart fills the figure instead of falling back to Vega-Lite's 200px default.
- Never paste the spec into a `<pre>` or `<code>` block as the deliverable. That is a debugging view, not a chart.

**WeasyPrint / PDF caveat.** WeasyPrint does not execute JavaScript, so the live-embed path renders to an empty `<div>` in PDF output. For documents that ship as PDF (most kami docs), pre-render the chart to PNG or SVG and embed an `<img>` inside the `<figure>`:
```bash
vl-convert vl2png --input chart.vl.json --output chart.png --scale 2
# or, with the Python toolchain:
python -m vl_convert vl2svg chart.vl.json chart.svg
```
Keep the live `<script>` version in the HTML deliverable so the chart is interactive on the web; the PDF gets the rasterised version. If only HTML is being shipped, the four-part live-embed pattern is all you need.
